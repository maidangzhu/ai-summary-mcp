...
export const loadConfig = async (): Promise<Config> => {
  try {
    ...
    return {
      ...
      analysis: { ...DEFAULT_ANALYSIS_CONFIG, ...config.analysis }
    };
  } catch (error) {
...
// 更新AI配置
export const updateAIConfig = async (aiConfig: Partial<AIProviderConfig>): Promise<void> => {
  const config = await loadConfig();
...
// 更新分析配置
export const updateAnalysisConfig = async (analysisConfig: Partial<AnalysisConfig>): Promise<void> => {
  const config = await loadConfig();
  config.analysis = { ...config.analysis, ...analysisConfig };
  await saveConfig(config);
};
